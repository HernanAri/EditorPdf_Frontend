...
export class AppConfigurator {
    ...
    ngOnInit() {
        ...
        //         // Restaurar valores en tu layoutService
        //         this.layoutService.layoutConfig.update(() => ({
        //             ...this.layoutService.layoutConfig(),
        //             ...parsed
        //         }));

        //         // Reaplicar tema con esos valores
        //         this.onPresetChange(parsed.preset);
        //         if (parsed.primary) {
        //             this.applyTheme('primary', { name: parsed.primary, palette: this.primaryColors().find(c => c.name === parsed.primary)?.palette });
        //         }
        //         if (parsed.surface) {
        //             this.applyTheme('surface', this.surfaces.find(s => s.name === parsed.surface));
        //         }
        //     } else {
        //         // Si no hay nada guardado, aplicamos la configuración por defecto
        //         this.onPresetChange(this.layoutService.layoutConfig().preset);
        //     }
        if (isPlatformBrowser(this.platformId)) {
            const savedConfig = localStorage.getItem('layoutConfig');
            ...
            if (savedConfig) {
                ...
                // Restaurar valores en tu layoutService (incluye darkTheme)
                this.layoutService.layoutConfig.update(() => ({
                    ...this.layoutService.layoutConfig(),
                    ...parsed
                }));

                // Reaplicar el tema con esos valores
                this.onPresetChange(parsed.preset);
                if (parsed.primary) {
                    this.applyTheme('primary', { name: parsed.primary, palette: this.primaryColors().find(c => c.name === parsed.primary)?.palette });
                }
                ...
                // 🔹 Aplica dark o light directamente en el body
                document.body.classList.toggle('dark-theme', parsed.darkTheme);
            } else {
                this.onPresetChange(this.layoutService.layoutConfig().preset);
            }
        }
